...
public:
  Label(const string &info) : info(info)
  {
  }
  ...
  string click(int x, int y) override
  {
    return "Label(" + info + ")";
  }
};
...
public:
  CheckBox(const string &txt, const bool status) : info(txt), flag(status)
  {
  }
